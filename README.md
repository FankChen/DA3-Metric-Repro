...
#### 自训 ckpt 在官方测试集上的最终评测

| 来源 | KITTI AbsRel / d1 | NYUv2 AbsRel / d1 |
|---|---|---|
| 论文 | 0.086 / 0.953 | 0.070 / 0.963 |
| 官方 DA3METRIC-LARGE (Path A) | 0.094 / 0.926 | 0.080 / 0.948 |
| 自训 v0 (KITTI-only, step_046000) | **0.065 / 0.952** ⭐ | 2.71 / 0.007 ❌ |
| **自训 v1 (KITTI+NYU mix, final)** | **0.065 / 0.951** ⭐ | **0.100 / 0.908** ✅ |

**v0 → v1 变化解读**：
- KITTI 几乎不变（0.0652→0.0654, d1 0.952→0.951）：一半 batch 用于 NYU 没有伤害 KITTI 的表现，因为暖启动 + 低 lr 收敛已稳定
- NYU 从完全崩溃 (AbsRel 2.71, d1 0.007) → 接近论文水平 (0.100, 0.908)。混训彻底解决了 domain shift 问题
- v1 KITTI 仍优于官方权重，NYU 还差官方约 25%（论文用了更大数据 + 更长训练）

#### 第二轮训练 v1（job 12223245，2026-04-26 22:43–23:06）

- **总时长**：24 分钟 / 23156 步（4 epoch，KITTI+NYU 50/50 混训）
- **暖启动**：从 v0 step_046000 加载（missing/unexpected = 0）
- **LR**：3e-5（比 v0 低，因为已收敛）
- **混训计数**：ds0(KITTI)=11525, ds1(NYU)=11616 — 几乎完美 50/50
- **Loss**：从 0.7 收敛到 0.7（已稳态）
- **ckpt**：`runs/da3metric_kn_v1/ckpts/final.pt` (1.9GB)
